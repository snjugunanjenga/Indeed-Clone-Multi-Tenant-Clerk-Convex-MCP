...
- `Job management UX includes optional auto-close behavior:`
  ...
  - `Jobs list includes edit navigation to app/company/jobs/[jobId]/edit/page.tsx.`
  - `Edit page supports updating title, description, location, types, compensation, tags, and auto-close toggle.`
...
- `Non-Convex automated verification completed:`
  - `pnpm lint passes.`
  - `pnpm build passes with all app routes compiled, including:`
    - `/pricing`
    - `/company`
    - `/company/jobs`
    - `/company/jobs/new`
    ...
    - `/company/billing`
    - `candidate routes (/jobs, /applications, /favorites, /profile)`
- `Manual browser smoke still required for auth/session-dependent flows:`
  - `candidate apply + company decision + notification visibility`
  - `role switch checks (admin/recruiter/member) across action buttons`
  - `no-org onboarding path in /pricing followed by org plan selection`
- `P5C status: in_progress (manual runtime smoke pending).`

#### `P5D: Deployment and data verification`

- `Use Convex MCP to confirm tables, functions, and expected data flow behavior.`
- `Temporary adjustment for this environment:`
  - `Skip Convex MCP reads while MCP tool calls are unstable/freezing.`
  - `Use app-level runtime validation (UI behavior + protected route behavior + successful build/lint) as interim check.`
  - `Re-run Convex table/function validation once Convex MCP is responsive.`
- `P5D status: in_progress (Convex MCP verification deferred).`
